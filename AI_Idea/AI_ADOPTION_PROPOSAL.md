...
```
 ...
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ AI自動レビューの試験導入                                  │
 │  → チェックリストを CLAUDE.md に変換                      │
 │  → 1リポジトリで PR に AI がコメントを自動投稿            │
 │  ✅ 成果：AI一次レビューが動く状態                        │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 精度評価と改善                                           │
 │  → AI指摘 vs 有識者指摘を比較してカバレッジを確認         │
 │  → 漏れているルールを CLAUDE.md に追記                    │
 │  ✅ 成果：主要な指摘パターンをAIがカバーできる状態         │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ 全リポジトリへ展開・定量計測                              │
 │  → 全リポジトリに AI レビューを展開                       │
 │  → 有識者のレビュー工数を月次で計測・継続改善              │
 │  ✅ 成果：レビュー待ち 50% 削減・有識者工数 40% 削減       │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ┌─ STEP 1（1ヶ月目）─────────────────────────────────────┐
 │ 評価基準の定量化                                         │
 │  → 有識者に「良いコード・悪いコード」の判断基準をヒアリング │
 │  → 複雑度・カバレッジ・規約遵守率などの指標に変換          │
 │  ✅ 成果：定量評価ルールの初版完成                         │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ 過去コードへの試験適用                                    │
 │  → 過去コードにルールを適用してスコアリング               │
 │  → 有識者の直感評価とスコアのズレを確認・調整              │
 │  ✅ 成果：有識者が納得できる評価ルールの確立               │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 新規コードへの適用・CI/CD組み込み                         │
 │  → スコアが基準を下回る PR を自動でブロック               │
 │  ✅ 成果：品質ゲートが機能する状態                        │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ 品質ダッシュボードの整備                                  │
 │  → コードメトリクスを可視化するダッシュボードを構築        │
 │  → 月次でチームの品質トレンドを共有                       │
 │  ✅ 成果：チーム品質の継続的なモニタリング体制             │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ...
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ 最小構成パイプラインの構築（ビルドのみ）                   │
 │  → まず「push で自動ビルドが走る」状態を最速で作る         │
 │  → チームが CI/CD の基本的な使い方に慣れる               │
 │  ✅ 成果：自動ビルドが動く状態                            │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 規約チェック・テストの追加                                │
 │  → 静的解析（MISRA-C）・ユニットテストをパイプラインに追加 │
 │  → カバレッジ計測・レポート生成も組み込む                  │
 │  ✅ 成果：品質チェックが全自動化された状態                 │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ AI レビューの統合・全リポジトリ展開                       │
 │  → PR に AI レビューコメントが自動投稿される              │
 │  → 全リポジトリへ展開                                     │
 │  ✅ 成果：テスト工数の大幅削減・品質チェックの完全自動化   │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ┌─ STEP 1（1ヶ月目）─────────────────────────────────────┐
 │ AI コード補完ツールの全員試用                             │
 │  → VS Code + GitHub Copilot を全員がインストール          │
 │  → 2週間試用後に「どの作業で効果があったか」を共有         │
 │  ✅ 成果：AI 補完の効果を体感・チームで共有               │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ CLAUDE.md（プロジェクトルール）の整備                     │
 │  → コーディング規約・アーキテクチャ・禁止パターンを定義    │
 │  → AI がプロジェクト固有のルールを理解した状態にする        │
 │  ✅ 成果：AI が規約準拠コードを生成できる環境              │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 影響範囲調査への AI 活用                                  │
 │  → 変更ファイルを入力すると AI が影響箇所を列挙するフローを試作│
 │  → 影響調査の所要時間を計測して効果を確認                  │
 │  ✅ 成果：影響調査工数の大幅削減                           │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ ツールの標準化・効果の定量計測                            │
 │  → 使用ツールを組織として標準化・ライセンス整備            │
 │  → コーディング時間の削減率を定期計測して継続改善          │
 │  ✅ 成果：開発環境の標準化とコーディング工数の定量削減      │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ...
 │ 使用 OSS の棚卸し                                        │
 │  → 使用している OSS をリスト化                            │
 │  → 更新頻度・影響範囲の大きさで優先順位を付ける            │
 │  ✅ 成果：対応優先度の高い OSS リスト                     │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ AI 差分解析フローの試作（1 OSS）                          │
 │  → リリースノート・差分を AI に読み込ませて影響箇所を列挙  │
 │  → 1つの OSS で試験的に実施して精度を確認                 │
 │  ✅ 成果：影響調査フローの確立                            │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ AI による修正コード生成の追加                             │
 │  → 影響箇所に対して AI が修正候補コードを生成             │
 │  → エンジニアがレビュー・修正して適用                      │
 │  ✅ 成果：OSS 対応リードタイムの大幅短縮                   │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ 対象 OSS の拡大・定期チェックの自動化                     │
 │  → 全 OSS に展開                                         │
 │  → 新バージョンのリリースを自動検知して通知               │
 │  ✅ 成果：セキュリティパッチへの迅速な対応体制             │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ...
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ RAG システムの PoC 構築                                   │
 │  → 主要ドキュメントを AI に読み込ませて Q&A を試験         │
 │  → 回答の精度・出典の正確さを評価                         │
 │  ✅ 成果：「聞けば答えてくれる」AI の動作確認              │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 特定チームでの試験運用                                    │
 ...
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ 対象ドキュメントの拡大・コードとのリンク化                 │
 │  → 全設計ドキュメントをナレッジベースに統合               │
 │  → コードとドキュメントを相互参照できる状態にする          │
 │  ✅ 成果：組織のナレッジが一元検索できる状態              │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ...
 │  → 全員が同じ運用ルールで入力できる状態を作る             │
 │  ✅ 成果：JIRA データ品質の向上                           │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ AI による進捗サマリーの自動生成                           │
 │  → JIRA データを週次で AI がサマリーして Slack/Teams に投稿│
 │  → 内容の精度・有用性をフィードバックして改善             │
 │  ✅ 成果：週次レポートの自動化                            │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ チケット自動分類・優先度スコアリング                      │
 │  → AI が新規チケットを種別・優先度で自動分類              │
 │  → 優先度の高い課題を自動でエスカレーション               │
 │  ✅ 成果：チケット整理の工数削減・対応漏れの防止           │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ リスク検知の自動化                                        │
 │  → 期限超過の予兆・ボトルネックを AI が自動検知           │
 │  → 問題が大きくなる前に担当者へ通知                       │
 │  ✅ 成果：プロジェクトリスクの早期発見体制                 │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ┌─ STEP 1（1ヶ月目）─────────────────────────────────────┐
 │ SharePoint ドキュメントの棚卸し                            │
 │  → 最新版の特定・旧版のアーカイブ化・命名規則の統一        │
 │  ✅ 成果：信頼できるドキュメントの整理                     │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ Doxygen コメントの AI 自動生成を全員に試験導入            │
 │  → 関数・モジュールの Doxygen コメントを AI が生成        │
 │  → コードから API ドキュメントが自動生成される状態を作る  │
 │  ✅ 成果：コメント作成工数の削減                           │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 設計変更時の仕様書ドラフト自動生成                        │
 │  → 変更差分を入力すると AI が更新箇所のドラフトを出力      │
 │  → エンジニアがレビュー・修正して SharePoint に反映        │
 │  ✅ 成果：仕様書更新工数の大幅削減                         │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ CI/CD と SharePoint の連携                                │
 │  → コードマージのタイミングでドキュメントが自動更新        │
 │  ✅ 成果：コードとドキュメントが常に同期された状態         │
 └─────────────────────────────────────────────────────────┘
```
...
```
 ┌─ STEP 1（1ヶ月目）─────────────────────────────────────┐
 │ 後発組のAIリテラシー現状把握                              │
 │  → 現在の AI 活用状況をヒアリング・アンケート             │
 │  → まず日常業務での AI 活用（補完・文書生成）を定着させる  │
 │  ✅ 成果：チームのAIリテラシー現状の把握                  │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ AIエージェント・ハンズオン研修（月1〜2回）                │
 │  → Claude Code / Copilot Studio でシンプルなエージェントを作る│
 │  → 「自分で作れた」体験を全員が得られるようにする          │
 │  ✅ 成果：エージェント作成の基礎スキル習得                │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 各自の業務にエージェントを適用                            │
 │  → 「自動化したい繰り返し作業」を1つ選んでエージェント化   │
 │  → 実際の業務で効果を体感する                             │
 │  ✅ 成果：個人レベルでの業務自動化の実現                  │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
...
```
 ┌─ STEP 1（1ヶ月目）─────────────────────────────────────┐
 │ AI コード生成ツールの全員試用                             │
 │  → GitHub Copilot / Claude Code を試用                   │
 │  → 「AI が得意な作業」と「人間が判断すべき作業」を把握     │
 │  ✅ 成果：AIとの役割分担の理解                            │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 2（2ヶ月目）─────────────────────────────────────┐
 │ CLAUDE.md による AI のコンテキスト整備                    │
 │  → コーディング規約・アーキテクチャ・禁止パターンを定義    │
 │  → AI が規約準拠のコードを生成できる前提条件を整える        │
 │  ✅ 成果：プロジェクト固有のルールを理解した AI の構築      │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 3（3ヶ月目）─────────────────────────────────────┐
 │ 定型処理コード生成テンプレートの整備                      │
 │  → 初期化・エラーハンドリング・ドライバ呼び出しのテンプレートを整備│
 │  → 類似処理の実装を AI で一気に自動化                     │
 │  ✅ 成果：定型処理のコーディング時間がほぼゼロ             │
 └──────────────────────────────┬──────────────────────────┘
                                ↓
 ┌─ STEP 4（4ヶ月目〜）───────────────────────────────────┐
 │ 設計書 → コード生成フローの確立                           │
 │  → 仕様・設計書を入力すると AI がコード初稿を生成         │
 │  → 生成コードの品質を CI/CD で自動検証                    │
 │  ✅ 成果：設計書からコードまでの一気通貫した自動化         │
 └─────────────────────────────────────────────────────────┘
```
